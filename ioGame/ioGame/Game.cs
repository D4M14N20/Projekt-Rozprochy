...
namespace ioGame
{
    internal class Game
    {
        ...
        public static Vector2 CameraPosition { get; private set; } = new Vector2(0, 0);
            ...
            if (NewClient.Connected)
            {
                lock (NewClient.PlayerStates)
                {
                    ...
                    foreach (var kv in NewClient.PlayerStates)
                        PlayerState.Set(otherPlayers[kv.Key], kv.Value);
                ...
                lock (NewClient.ReceivedEvents)
                {
                    while (NewClient.ReceivedEvents.Count > 0)
                    {
                        ...
                        if (otherPlayers.ContainsKey(evnt.sender))
                        {
                            ...
                            if (evnt.gameEvent == GameEvents.SizeSet)
                            {
                            ...
                            else if (evnt.gameEvent == GameEvents.BulletSpawn)
                            {
                                ...
                                int id = (int)(long)evnt.eventsArgs[3];

                                otherPlayers[evnt.sender].idMap[id] = bullet.Id;
                            }
                            else if (evnt.gameEvent == GameEvents.BulletHitted)
                            {
                                //otherPlayers[evnt.sender].PlayerName = evnt.eventsArgs[0].GetType().Name;
                                int id = (int)(long)evnt.eventsArgs[0];
                                if (otherPlayers[evnt.sender].idMap.ContainsKey(id))
                                {
    ...
    {
        public static Vector2 GetScreenPosition(double camerax, double cameray, Size size, double posx, double posy, double scale = 1.0f)
        {
